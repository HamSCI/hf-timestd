...
class Phase2AnalyticsService:
    ...
    def _write_test_signal(self, minute_boundary: int, iq_samples: np.ndarray, minute_number: int):
        ...
        try:
            ...
            logger.info(
                ...
                f"confidence={detection.confidence:.2f if detection.confidence else 0:.2f}, "
                f"station={station}"
            )
            ...
            # ================================================================
            if not self.enable_hdf5_writes:
                logger.warning(f"{self.channel_name}: HDF5 writes disabled, skipping test signal write")
                return
            if not self.hdf5_l2_test_signal_writer:
                logger.warning(f"{self.channel_name}: No HDF5 test signal writer, skipping test signal write")
                return
    ...
    def _read_binary_minute(self, target_minute: int):
        ...
        try:
            ...
            # CRITICAL FIX (2026-01-12): Calculate actual Unix time from metadata
            # 1. Prefer start_system_time if provided by recorder (atomic metadata)
            # 2. Otherwise learn/drift-protect the offset
            if json_path.exists() and 'start_rtp_timestamp' in metadata:
                ...
                if start_system_time is not None:
                    ...
                    inst_offset = start_system_time - (rtp_timestamp / self.sample_rate)
                else:
    ...
    def _get_latest_minute(self) -> int:
        ...
        """
        now = time.time()
        # Go back 4 minutes for safety (data completeness and atomic write handling)
        latest_minute = ((int(now) // 60) - 4) * 60
