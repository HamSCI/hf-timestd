...
class TestScintillationIndices:
    ...
    def test_s4_moderate_scintillation(self, analyzer):
        ...
        n_samples = 60
        mean_amp = 1.0
        # Moderate variance: std = 0.4 * mean → S4 ≈ 0.4
        amplitudes = mean_amp + 0.4 * mean_amp * np.random.randn(n_samples)
    ...
    def test_sigma_phi_weak(self, analyzer):
        ...
        # Linear phase (Doppler) with small fluctuations
        # After detrending, σ_φ should be small
        doppler_hz = 0.5
        ...
        assert abs(result.doppler_removed_hz - doppler_hz) < 0.1
    ...
    def test_doppler_removal(self, analyzer):
        ...
        # Pure Doppler (linear phase) with known rate
        doppler_hz = 1.5
...
class TestScintillationPhysics:
    ...
    def test_periodic_fading(self, analyzer):
        ...
        mean_amp = 1.0
        fade_depth = 0.5  # 50% modulation depth
        amplitudes = mean_amp * (1 + fade_depth * np.sin(2 * np.pi * times / 20))
        phases = np.zeros(n_samples)
        
        result = analyzer.calculate_scintillation_indices(amplitudes, phases, times)
        
        # For sinusoidal fading, S4 ≈ fade_depth / sqrt(2) ≈ 0.35
        expected_s4 = fade_depth / np.sqrt(2)
        assert abs(result.s4_index - expected_s4) < 0.1, \
            f"Periodic fading S4 mismatch: expected ~{expected_s4:.2f}, got {result.s4_index:.2f}"
