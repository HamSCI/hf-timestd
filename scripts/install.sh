...
# Generate per-channel .env files for metrology template instances
# radiod mode: 9 frequency channels; phase-engine: 17 broadcast channels
log_info "  Generating metrology channel environment files..."
METROLOGY_ENV_DIR="$CONFIG_DIR/metrology-channels"
mkdir -p "$METROLOGY_ENV_DIR"

# Read station info from config
CALLSIGN=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print(c['station']['callsign'])" 2>/dev/null)
GRID_SQUARE=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print(c['station']['grid_square'])" 2>/dev/null)
STATION_ID=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print(c['station']['id'])" 2>/dev/null)
INSTRUMENT_ID=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print(c['station']['instrument_id'])" 2>/dev/null)
LATITUDE=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print(c['station']['latitude'])" 2>/dev/null)
LONGITUDE=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print(c['station']['longitude'])" 2>/dev/null)

# Determine archive root (tiered storage uses /dev/shm hot buffer)
TIERED_STORAGE=$("$VENV_DIR/bin/python3" -c "
import tomllib
with open('$MAIN_CONFIG', 'rb') as f: c = tomllib.load(f)
print('true' if c.get('recorder', {}).get('tiered_storage', False) else 'false')" 2>/dev/null)
...
ENVEOF
    ...
    # Enable the template instance (WantedBy=timestd-metrology.target)
    systemctl enable "timestd-metrology@${CHANNEL}.service" 2>/dev/null || true
done

chown -R "$INSTALL_USER:$INSTALL_USER" "$METROLOGY_ENV_DIR"
log_info "    ✅ ${#METROLOGY_CHANNELS[@]} channel .env files in $METROLOGY_ENV_DIR"
log_info "    ✅ ${#METROLOGY_CHANNELS[@]} template instances enabled"
