...
# Read [ka9q] status from config (works for both local and remote radiod).
# Phase 6 cutover: canonical field name is `status` (was `status_address`).
RADIOD_STATUS_ADDR=""
if [[ -f "$CONFIG_FILE" ]]; then
    RADIOD_STATUS_ADDR=$(awk '/^\[ka9q\]/{found=1; next} /^\[/{found=0} found && /^status[[:space:]]*=/ {sub(/[^=]*=\s*/,""); sub(/#.*/,""); gsub(/^[\x27" \t]+|[\x27" \t]+$/,""); print; exit}' "$CONFIG_FILE" 2>/dev/null)
fi
