#!/usr/bin/env python3
"""
ARCHIVED 2026-03-06: This is the complete original wwvh_discrimination.py before
dead code removal. The weighted voting pipeline (compute_discrimination,
finalize_discrimination, _cross_validate_methods, etc.) is no longer called at
runtime — the TickEdgeDetector runs station-specific matched-filter templates in
parallel, making voting unnecessary. See SESSION_2026_03_06_DISCRIMINATION_AUDIT.md.

The live module retains only:
  ...
  - test_signal_detector (WWVTestSignalDetector sub-object)
  - __init__() with BCD encoder, geo predictor, and test signal detector setup

This archive preserves the full original for reference, including:
  - 8-method weighted voting algorithm documentation (lines 42-148)
  - DiscriminationResult dataclass with all fields
  - All dead methods: compute_discrimination, finalize_discrimination,
    detect_timing_tones, detect_440hz_tone, detect_500_600hz_tone,
    detect_tick_windows, measure_tone_powers_fft, estimate_doppler_shift,
    bcd_correlation_with_doppler_compensation, analyze_minute_with_440hz,
    _cross_validate_methods, get_recent_measurements, get_statistics
