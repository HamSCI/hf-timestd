...
T3 places the second boundary from the received tick train itself. The metrology folds the band-limited envelope at one second over 60 to 180 s, fits the peaks to the expected station delays by a common shift, and holds the result in the RTP frame, where the GPSDO keeps it constant within a counter epoch. The winning peak must also recur, independently, in a fold of each half of the acquiring buffer (task-11b, spec §10) — a guard against a fold-lattice phantom that only crosses the detection floor at one particular fold length — which costs roughly 1 dB of input sensitivity (measured: the synthetic acquisition floor moves from about −17 dB to about −16.5 dB) since each half only integrates half the rows. Each minute's ensembles corroborate or correct it; a counter-epoch change discards it. radiod's `(GPS_TIME, RTP_TIMESNAP)` pair now bounds only the whole second. Until acquisition the station reports `BOOTSTRAP` and promotes no timing. `/run/hf-timestd/registration.json` carries the acquired origin, its σ, the contributing channels and the raw-pair residual; the Offset Judge's `hf_acquired` bench reads it. A T6 station's own acquired plane runs alongside T6 rather than replacing it: it reports `WITNESS`, corroborating T6's plane instead of governing the published offset. Design: `docs/superpowers/specs/2026-09-06-t3-self-registration-design.md`.

`authority.json` surfaces this provenance under `"registration"`, built by `registration_block()` in `core/authority_manager.py`: `source` reads `"hf_acquired"` only in the `ACQUIRED` state, else `"label"`; `state` carries the station's registration state (`BOOTSTRAP`/`ACQUIRED`/`WITNESS`/`CONFLICT`/`STALE`); and `sigma_ms`, `counter_epoch_id`, `raw_pair_residual_ms`, `contributing`, and `stations` pass through from the summary (`residual_vs_t6_ms` joins them in the `WITNESS` state). Every block also carries `age_s`, the summary's age against the store's own clock — `None` when there is no summary at all. `read_summary()` has no staleness check of its own: a summary file, once written, reads back exactly as written however old it is. So `registration_block()` applies the store's `stale_s` window itself — once `age_s` exceeds it, the block reports `state="STALE"` and `source="label"` regardless of what the dead writer last claimed, while still carrying its last `counter_epoch_id`/`sigma_ms`/`contributing`/`stations` so a reader can see what died. A missing summary file (or one this station's `RegistrationStore` otherwise cannot read) degrades to the no-summary shape (`state="UNKNOWN"`, `age_s=None`) rather than raising, and either way a `RegistrationStore` failure never blocks the rest of `authority.json`. The fusion service's `/run/hf-timestd/timing_chain.json` (`core/timing_chain_publisher.py`) carries the identical block for the same reason.
