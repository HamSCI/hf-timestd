...
2. **Python 3.10+** (dependencies in `pyproject.toml`)
3. **Data being generated** by core recorder

### Installation

The Web UI is now part of the main `hf-timestd` package.

```bash
# In the project root, install everything
pip install .
```
...
# Or manually via uvicorn (once installed)
cd web-ui
uvicorn monitoring_server:app --port 8080 --host 0.0.0.0
```

Default data root is `/tmp/timestd-test` (test mode), unless configured otherwise.

### Accessing Dashboards

**Entry point**: <http://localhost:8080/> (redirects to summary)
...
```
web-ui/
├── monitoring_server.py         # ⭐ FastAPI App
